...
class RevenueService:
    ...
    async def get_total_vault_revenue_usd(
        ...
        """
        Get total vault revenue for the last tempo.

        Takes a fresh fee snapshot per job, diffs against the previous snapshot,
        and converts to USD. Called once per weight-setting cycle.

        Args:
            lookback_days: Unused (kept for API compatibility).
            pair_price_usd: Unused (kept for API compatibility).

        Returns:
            Total revenue in USD.
        """
        from validator.services.fee_tracker import get_tempo_revenue_usd
        ...
        return await get_tempo_revenue_usd(active_jobs)
